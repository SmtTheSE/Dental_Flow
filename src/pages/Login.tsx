  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (response) => {
      ...
      if (response.access_token && !response.credential) {
        // If we only have an access token, we need to exchange it for user info
        try {
          ...
          // Create a mock credential token with the user info
          ...
          setErrors('Failed to fetch user information from Google');
          return;
        }
      }
    ...
    onError: (error) => {
      console.error('Google login error:', error);
      setErrors(`Google login failed: ${error.error} - ${error.description}`);
    },
